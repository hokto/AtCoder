...
def main():
    ...
    choco.sort(key=lambda x:(-x[0],-x[1]))
    box = []
    for i in range(M):
        box.append((C[i],D[i]))
    box.sort(key=lambda x:(-x[0],-x[1]))
    box_idx = 0
    box_width = BinarySearchTree()
    isok = True
    for (a,b) in choco:
        while box_idx<M and a<=box[box_idx][0]:
            box_width.insert(box[box_idx][1])
            box_idx+=1
        lb_box_width = box_width.lower_bound(b)
        if lb_box_width is None:
            isok = False
            break
        box_width.delete(lb_box_width)
    if isok:
        print("Yes")
    else:
        print("No")

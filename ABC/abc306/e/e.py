...
def main():
    N,K,Q = myin_sp_i()
    bst1 = BinarySearchTree(init_tree=[0]*K) # K個選択した方
    bst2 = BinarySearchTree(init_tree=[0]*(N-K)) # 未選択の方
    A = [0]*N
    ans = 0
    for q in range(Q):
        x,y = myin_sp_i()
        x-=1
        exist_selected = bst1.is_contain(A[x])
        exist_not_selected = bst2.is_contain(A[x])
        if exist_selected and not exist_not_selected:
            ans-=A[x]
            bst1.delete(A[x])
            bst2.insert(y)
            mx = bst2.max()
            ...
            bst1.insert(mx)
            ans+=mx
        elif not exist_selected and exist_not_selected:
            bst2.delete(A[x])
            selected_min =  bst1.min()
            if selected_min < y:
                bst1.delete(selected_min)
                ans-=selected_min
                bst2.insert(selected_min)
                bst1.insert(y)
                ans+=y
            else:
                bst2.insert(y)

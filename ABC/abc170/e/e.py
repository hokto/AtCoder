...
def main():
    ...
    belonging = [-1]*N
    rating = [0]*N
    rating_st = SegTree(min,INF,I)
    infant_rating_pq = [BinarySearchTree() for _ in range(I)]
    for i in range(N):
        a,b = myin_sp_i()
        b-=1
        rating[i]=a
        belonging[i]=b
        infant_rating_pq[b].insert(a)
    for i in range(I):
        if not infant_rating_pq[i].is_empty():
            rating_st.set(i,infant_rating_pq[i].max())
    ...
    for q in range(Q):
        ...
        d-=1
        rate = rating[c]
        prev_belong = belonging[c]
        infant_rating_pq[prev_belong].delete(rate)
        v1 = infant_rating_pq[prev_belong].max()
        if v1>=0:
            rating_st.set(prev_belong,infant_rating_pq[prev_belong].max())
        else:
            rating_st.set(prev_belong,INF)
        belonging[c] = d
        infant_rating_pq[d].insert(rate)
        v2 = infant_rating_pq[d].max()
        if v2>=0:
            rating_st.set(d,infant_rating_pq[d].max())
        else:
            rating_st.set(d,INF)
        print(rating_st.all_prod())
